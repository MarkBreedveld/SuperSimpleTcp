   ...
   public class SimpleTcpServer : IDisposable
   {
      ...
      private async Task SendInternalAsync(string ipPort, long contentLength, Stream stream, CancellationToken token)
      {
         ...
         try
         {
            ...
            if (client == null) return;
            if (_settings.MaxPendingSendTaskPerClient > 0 && client.OutstandingJobs > _settings.MaxPendingSendTaskPerClient)
               throw new TimeoutException($"Client failed to keep up with server data stream in time. This timeout is measured by number of outstanding jobs. The current amount of jobs '{client.OutstandingJobs}' exceeded the maximum of '{_settings.MaxPendngSendsPerClient}' outstanding jobs.");
            client.SendStarted();
            long bytesRemaining = contentLength;
            int bytesRead = 0;
            byte[] buffer = new byte[_settings.StreamBufferSize];
